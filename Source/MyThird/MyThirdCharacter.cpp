...
AMyThirdCharacter::AMyThirdCharacter()
{
	...
	CameraBoom->SetRelativeRotation(FRotator(-80, 0.f, 0.f)); // 设定角度
...
void AMyThirdCharacter::BeginPlay()
{
	...
	AThirdPlayerController* ThirdPlayerController = Cast<AThirdPlayerController>(GetController());
	if (ThirdPlayerController != nullptr)
	{
		ThirdPlayerController->UpdateHealthPrecent(HealthComponent->GetHealthPercent());
	}
}
...
void AMyThirdCharacter::Death_Implementation()
{
	...
	if (ThirdPlayerController != nullptr)
	{
		ThirdPlayerController->CreateRestartWidget();
	}
}
